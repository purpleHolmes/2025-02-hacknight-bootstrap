...
class GithubListIssuesRequest(BaseModel):
    ...
    state: Literal["open", "closed", "all"] = Field(
        "open", description="The state of the issues to return"
    )
    ...
    number_of_issues: int | None = Field(
        default=None, description="The number of issues to return"
    )
    ...
    while req.number_of_issues is None or len(issues) < req.number_of_issues:
        ...
        for issue in page:
            ...
            page_issues.append({
                ...
                "title": raw["title"],
                "user": raw["user"]["login"],
                "assignees": [assignee["login"] for assignee in raw["assignees"]],
